# QWC edit 2023/08/16 GMT+8 19:00
...
from model.model import RankinglossModel_v6 as RankinglossModel # T5_model

from parse_config import ConfigParser
from trainer import MultiCrimeTrainer as Trainer
...
def main(config, config_path):
    ...
    final_model_input, extra_input = data_provider.prepare_data_LeCaRDv2(config, ex_id)
    ...
    train_loader, test_loader, extra_train_loader, join_train_loader = build_loaders(config, ex_id, model_tokenizer, 
                                            data_provider=data_provider,
                                            final_model_input=final_model_input,
                                            extra_input=extra_input,
                                            join=config["join"] if "join" in config else False,
                                        )
    ...
    trainer = Trainer(model, criterion, metrics_ftns, optimizer,
                      ...
                      ex_id=ex_id,
                      data_provider=data_provider,
                      join_train_loader=join_train_loader,
                      join=config["join"] if "join" in config else False,
                      join_epoch=config["join_epoch"] if "join_epoch" in config else 0,
    )
