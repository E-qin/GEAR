    ...
    def process(
        ...
        for batch_idx, beam_hyp in enumerate(self._beam_hyps):
            ...
            for beam_token_rank, (next_token, next_score, next_index) in enumerate(
                ...
                # print("next_score\t", next_score) # QWC check
                # print("next_score.requires_grad\t", next_token.requires_grad) # QWC check
...
class BeamSearchScorerGrad(BeamScorer):
    r"""
    [`BeamScorer`] implementing standard beam search decoding.
        ...
        for batch_idx, beam_hyp in enumerate(self._beam_hyps):
            ...
            for beam_token_rank, (next_token, next_score, next_index) in enumerate(
                ...
                if (eos_token_id is not None) and (next_token.item() == eos_token_id):
                    ...
                    beam_hyp.add(
                        input_ids[batch_beam_idx].clone(),
                        next_score,                                                      # QWC change for Grad: del .item(), original: 'next_score.item()'
                        beam_indices=beam_index,
                    )
                else:
                    # add next predicted token since it is not eos_token
    ...
    def finalize(
        ...
        for batch_idx, beam_hyp in enumerate(self._beam_hyps):
            ...
            for beam_id in range(self.num_beams):
                batch_beam_idx = batch_idx * self.num_beams + beam_id
                final_score = final_beam_scores[batch_beam_idx]                     # QWC change for Grad: del .item(), original: 'final_beam_scores[batch_beam_idx].item()'
                final_tokens = input_ids[batch_beam_idx]
                beam_index = beam_indices[batch_beam_idx] if beam_indices is not None else None
                beam_hyp.add(final_tokens, final_score, beam_indices=beam_index)    
...
class BeamHypothesesGrad:
    ...
    def add(self, hyp: torch.LongTensor, sum_logprobs: torch.LongTensor, beam_indices: Optional[torch.LongTensor] = None): # QWC change for Grad, original: 'sum_logprobs: float'
        """
        Add a new hypothesis to the list.
        """
        score = sum_logprobs / (hyp.shape[-1] ** self.length_penalty) 
        if len(self) < self.num_beams or score.item() > self.worst_score: # QWC change for Grad: add .item(), original: 'score > self.worst_score'
            self.beams.append((score, hyp, beam_indices))
            if len(self) > self.num_beams:
                sorted_next_scores = sorted([(s.item(), idx) for idx, (s, _, _) in enumerate(self.beams)]) # QWC change for Grad: add .item(), original: 'sorted([(s, idx) for idx, (s, _, _) in enumerate(self.beams)])'
                del self.beams[sorted_next_scores[0][1]]
                self.worst_score = sorted_next_scores[1][0]
            else:
                self.worst_score = min(score.item(), self.worst_score) # QWC change for Grad: add .item(), original 'self.worst_score = min(score, self.worst_score)'
