...
from .generation_beam_search import BeamScorer, BeamSearchScorer, ConstrainedBeamSearchScorer, BeamSearchScorerGrad
...
class GenerationMixin:
    ...
    def generate_grad(
        ...
        **model_kwargs,
        ...
        # 9. go into different generation modes
        if is_greedy_gen_mode:
            raise ValueError("Not implemented yet, is_greedy_gen_mode")
            # if num_return_sequences > 1:
            #     raise ValueError(
            #         f"num_return_sequences has to be 1, but is {num_return_sequences} when doing greedy search."
            #     )
            ...
            raise ValueError("Not implemented yet, is_sample_gen_mode")
            # # 10. prepare logits warper
            # logits_warper = self._get_logits_warper(
            #     top_k=top_k,
            #     top_p=top_p,
            ...
            # 10. prepare beam search scorer
            beam_scorer = BeamSearchScorerGrad(      # QWC change for Grad, original: 'beam_scorer = BeamSearchScorer('
                batch_size=batch_size,
                num_beams=num_beams,
                device=inputs_tensor.device,
                length_penalty=length_penalty,
            ...
            # 12. run beam search
            return self.beam_search(  # QWC change for Grad, original: 'return self.beam_search('
                input_ids,
                beam_scorer,
                logits_processor=logits_processor,
                stopping_criteria=stopping_criteria,
            ...
            raise ValueError("Not implemented yet, is_beam_sample_gen_mode")
            # 10. prepare logits warper
            # logits_warper = self._get_logits_warper(
            #     top_k=top_k,
            #     top_p=top_p,
            ...
            raise ValueError("Not implemented yet, is_group_beam_gen_mode")
            # if num_return_sequences > num_beams:
            #     raise ValueError("`num_return_sequences` has to be smaller or equal to `num_beams`.")
            ...
            raise ValueError("Not implemented yet, is_constraint_gen_mode")
            # if num_return_sequences > num_beams:
            #     raise ValueError("`num_return_sequences` has to be smaller or equal to `num_beams`.")
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # print("outputs.logits\t",outputs.logits.grad_fn,outputs.logits.requires_grad) # QWC check
            ...
            # print("next_token_scores", next_token_scores.grad_fn, next_token_scores.requires_grad) # QWC check
            ...
            # print("beam_scores.grad_fn",beam_outputs["next_beam_scores"].grad_fn) # QWC check
        ...
        # print("sequence_outputs['sequence_scores'].grad_fn\t",sequence_outputs["sequence_scores"].grad_fn) # QWC check
        if return_dict_in_generate:
